...
| `src/lib/commerce/*`, `src/app/api/{checkout,snack,webhooks}/**`, `supabase/schema.sql`, `src/app/checkout/success/page.tsx` | | Stripe Checkout Sessions, verified webhooks, Supabase orders, print-on-demand adapter with a **demo implementation** when env is unset. |
