...
| `src/components/story/*` | `default StoryEnd()`, frame loading | `StoryEnd` = Turntable + the two big buttons. `frameLoader.ts` (limiter, manifests, decode — in two Web Workers when the browser allows, because Safari decodes `createImageBitmap` on the main thread) and `useFrameSequence.ts` (`SequenceController`, windowed: every frame's bytes kept, only ±10 frames around `focus(i)` plus a keyframe every 12 decoded) is the chapter reader's frame source. |
