...
| `src/components/story/*` | `default StoryEnd()`, frame loading | `StoryEnd` = Turntable + the two big buttons. `frameLoader.ts` (limiter, manifests, decode — in two Web Workers when the browser allows, because Safari decodes `createImageBitmap` on the main thread) and `useFrameSequence.ts` (`SequenceController`) are shared by SwipeStory, the chapter reader and the turntable. |
