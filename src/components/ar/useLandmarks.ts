...
import { LANDMARK_KEYS, POSE_INDEX, type Landmarks } from "./landmarks";
...
export function useLandmarks(
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      try {
        poses = landmarker.detectForVideo(video, t0).landmarks;
      } catch (err) {
        errors += 1;
      ...
      const prev = landmarks.current;
      if (!prev) {
        const fresh: Partial<Landmarks> = {};
        for (const key of LANDMARK_KEYS) {
          const raw = pose[POSE_INDEX[key]];
          fresh[key] = { x: raw.x, y: raw.y, visibility: raw.visibility ?? 1 };
        }
        landmarks.current = fresh as Landmarks;
      } else {
        for (const key of LANDMARK_KEYS) {
          const raw = pose[POSE_INDEX[key]];
          const p = prev[key];
